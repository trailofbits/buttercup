...
class DockerInteractive: 
    ...
    def __init__(self, container_image: str, mount_dirs: dict[Path, Path], start_command: list[str], global_timeout: float = 600.0):
        ...
        logger.info("Initializing DockerInteractive session: container=%s, timeout=%.1fs", container_image, global_timeout)
        ...
        try:
            ...
            if self.docker_process:
                ...
                if returncode is not None:
                    logger.warning("Docker process exited with returncode=%d (read %d lines before exit)", returncode, line_count)
                else:
                    logger.debug("Output reader thread finished, read %d lines total (process still running)", line_count)
            else:
    ...
    def _check_timeout(self):
        ...
        self.out_q.put(f"*** Global timeout exceeded after {elapsed_time:.1f} seconds (limit: {self.global_timeout:.1f}s) ***")
    ...
    def send_command(self, command: str, completion: CompletionFn, timeout: float = 10.0) -> CommandResult:
        ...
        returncode = self.docker_process.poll()
        if returncode is not None:
            logger.error("Docker process has already exited with returncode=%d, cannot send command: %s", returncode, command)
