...
SUPPORTED_MEMORY_TOOLS_FOR_OPTIONS = ["HWASAN", "ASAN", "KASAN", "CFI", "MSAN", "TSAN", "UBSAN", "NOSANITIZER"]
...
def join_memory_tool_options(options):
    ...
    the environment."""
    return ":".join("%s=%s" % (key, str(value)) for key, value in sorted(six.iteritems(options)))
...
def get_asan_options(redzone_size, malloc_context_size, quarantine_size_mb, bot_platform, leaks, disable_ubsan):
...
def get_default_tool_path(tool_name):
    ...
    tool_path = os.path.join(get_platform_resources_directory(platform_override), tool_filename)
    return tool_path
...
def get_sanitizer_options_for_display():
    ...
    for tool in SUPPORTED_MEMORY_TOOLS_FOR_OPTIONS:
        ...
        if not options_value:
            continue
        result.append(f"{options_variable}={options_value}")
...
def get_llvm_symbolizer_path():
    ...
    if llvm_symbolizer_path and os.path.exists(llvm_symbolizer_path):
        ...
        return_code = subprocess.call(
            [llvm_symbolizer_path, "--help"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        if return_code == 0:
...
def get_suppressions_file(sanitizer, suffix="suppressions"):
    ...
    sanitizer_suppressions_filename = f"{sanitizer}_{suffix}.txt"
    sanitizer_suppressions_file_path = os.path.join(get_suppressions_directory(), sanitizer_suppressions_filename)
...
def is_trusted_host(ensure_connected=True):
    ...
    return get_value("TRUSTED_HOST") and (not ensure_connected or get_value("WORKER_BOT_NAME"))
...
def is_running_on_app_engine_development():
    ...
    return os.getenv("GAE_ENV") == "dev" or os.getenv("SERVER_SOFTWARE", "").startswith("Development/")
...
def update_symbolizer_options(tool_options, symbolize_inline_frames=False):
    ...
    if "external_symbolizer_path" not in tool_options:
        ...
        if llvm_symbolizer_path:
            tool_options.update({"external_symbolizer_path": _quote_value_if_needed(llvm_symbolizer_path)})
    if "symbolize_inline_frames" not in tool_options:
        tool_options.update({"symbolize_inline_frames": str(symbolize_inline_frames).lower()})
...
def set_tsan_max_history_size():
    ...
    for i in range(tsan_max_history_size):
        tsan_options = tsan_options.replace("history_size=%d" % i, "history_size=%d" % tsan_max_history_size)
