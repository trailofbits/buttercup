...
ANDROID_KERNEL_ERROR_REGEX = re.compile(r".*Internal error: (Oops)?( -|:) (BUG|[0-9a-fA-F]+)")
...
ASAN_REGEX = re.compile(r".*ERROR: (HWAddressSanitizer|AddressSanitizer)[: ]*[ ]*([^(:;]+)")
ASSERT_REGEX = re.compile(
    r"(?:\[.*?\]|.*\.(?:%s):.*)?" % ("|".join(C_CPP_EXTENSIONS)) + r"\s*(?:ASSERT(?:ION)? FAIL(?:URE|ED)|panic): (.*)",
    re.IGNORECASE,
)
ASSERT_REGEX_GOOGLE = re.compile(GOOGLE_LOG_FATAL_PREFIX + r".*assertion failed at\s.*\sin\s*.*: (.*)")
ASSERT_REGEX_GLIBC = re.compile(r'.*:\s*assertion [`\'"]?(.*?)[`\'"]? failed\.?$', re.IGNORECASE)
...
ASSERT_REGEX_GLIBC_SUFFIXED = re.compile(r".*\S.*\/.*:\d+:\s*assertion .* failed:\s*(\S.*)")
ASSERT_NOT_REACHED_REGEX = re.compile(r"^\s*SHOULD NEVER BE REACHED\s*$")
CENTIPEDE_TIMEOUT_REGEX = re.compile(r"^========= Timeout of \d+ seconds exceeded; exiting")
...
CFI_NODEBUG_ERROR_MARKER_REGEX = re.compile(r"CFI: Most likely a control flow integrity violation;.*")
CHROME_CHECK_FAILURE_REGEX = re.compile(r"\s*\[[^\]]*[:]([^\](]*).*\].*Check failed[:]\s*(.*)")
...
MSAN_TSAN_REGEX = re.compile(r".*(ThreadSanitizer|MemorySanitizer):\s+(?!ABRT)(?!ILL)([^(:]+)")
EXTRA_SANITIZERS_COMMAND_INJECTION_REGEX = re.compile(r"===BUG DETECTED: Shell (corruption|injection)===")
EXTRA_SANITIZERS_ARBITRARY_FILE_OPEN_REGEX = re.compile(r"===BUG DETECTED: Arbitrary file open===")
EXTRA_SANITIZERS_ARBITRARY_DNS = re.compile(r"===BUG DETECTED: Arbitrary domain name resolution===")
EXTRA_SANITIZERS_PYSECSAN = re.compile(r"===BUG DETECTED: PySecSan:")
FATAL_ERROR_GENERIC_FAILURE = re.compile(r"#\s+()(.*)")
FATAL_ERROR_CHECK_FAILURE = re.compile(r"#\s+(Check failed: |RepresentationChangerError: node #\d+:)(.*)")
...
GOOGLE_CHECK_FAILURE_REGEX = re.compile(GOOGLE_LOG_FATAL_PREFIX + r"\s*Check failed[:]\s*(.*)")
...
JAZZER_JAVA_SECURITY_EXCEPTION_REGEX = re.compile("== Java Exception: .*FuzzerSecurityIssue")
...
KASAN_ACCESS_TYPE_ADDRESS_REGEX = re.compile(r"(Read|Write) of size ([0-9]+) at (addr|address) ([a-f0-9]+)")
KASAN_CRASH_TYPE_ADDRESS_REGEX = re.compile(r"BUG: KASAN: (.*) (in|on).*(addr|address) ([a-f0-9]+)")
KASAN_CRASH_TYPE_ADDRESS_RANGE_REGEX = re.compile(r"KASAN: (.*?) (in|on) range \[([a-z0-9]+)-([a-z0-9]+)\]")
KASAN_CRASH_TYPE_FUNCTION_REGEX = re.compile(r"BUG: KASAN: (.*) (in|on).* ([\w]+)\+([\w]+)\/([\w]+)")
...
LIBFUZZER_FUZZ_TARGET_EXITED_REGEX = re.compile(r".*ERROR:\s*libFuzzer:\s*fuzz target exited")
LIBFUZZER_OVERWRITES_CONST_INPUT_REGEX = re.compile(r".*ERROR:\s*libFuzzer:\s*fuzz target overwrites its const input")
...
SAN_CRASH_TYPE_ADDRESS_REGEX = re.compile(r"[ ]*([^ ]*|Atomic [^ ]*) of size ([^ ]*) at ([^ ]*)")
SAN_DEADLYSIGNAL_REGEX = re.compile(r"(Address|Leak|Memory|UndefinedBehavior|Thread)Sanitizer:DEADLYSIGNAL")
CONCATENATED_SAN_DEADLYSIGNAL_REGEX = re.compile(r"\n([^\n]*\S[^\n]*)(" + SAN_DEADLYSIGNAL_REGEX.pattern + r")\n")
...
SAN_SEGV_CRASH_TYPE_REGEX = re.compile(r".*The signal is caused by a ([A-Z]+) memory access.")
...
SECURITY_CHECK_FAILURE_REGEX = re.compile(r".*\[[^\]]*[:]([^\](]*).*\].*Security CHECK failed[:]\s*(.*)\.\s*")
SECURITY_DCHECK_FAILURE_REGEX = re.compile(r".*\[[^\]]*[:]([^\](]*).*\].*Security DCHECK failed[:]\s*(.*)\.\s*")
...
UBSAN_INVALID_BOOL_VALUE_REGEX = re.compile(r".*not a valid value for type \'(bool|BOOL)\'.*")
...
UBSAN_NO_RETURN_VALUE_REGEX = re.compile(r".*reached the end of a value-returning function.*")
UBSAN_NULL_ARGUMENT_REGEX = re.compile(r".*null pointer passed as .*, which is declared to never be null.*")
UBSAN_NULL_POINTER_READ_REGEX = re.compile(r".*load of null pointer.*")
UBSAN_NULL_POINTER_REFERENCE_REGEX = re.compile(r".*(binding to|access within|call on) null pointer.*")
UBSAN_NULL_POINTER_WRITE_REGEX = re.compile(r".*store to null pointer.*")
UBSAN_OBJECT_SIZE_REGEX = re.compile(r".*address .* with insufficient space for an object of type.*")
...
UBSAN_UNREACHABLE_REGEX = re.compile(r".*execution reached an unreachable program point.*")
UBSAN_VLA_BOUND_REGEX = re.compile(r".*variable length array bound evaluates to non-positive value.*")
...
UBSAN_VPTR_INVALID_OFFSET_REGEX = re.compile(r".*at offset (\d+) within object of type (.*)")
...
V8_CORRECTNESS_METADATA_REGEX = re.compile(r"#\s*V8 correctness ((configs|sources|suppression): .*)")
...
WINDOWS_CDB_STACK_OVERFLOW_REGEX = re.compile(r".*ExceptionCode: .*\(Stack overflow\).*")
WINDOWS_SAN_ILL_REGEX = re.compile(r".*[a-zA-Z]+Sanitizer: illegal-instruction")

WYCHEPROOF_JAVA_EXCEPTION = re.compile(r".*\) (.*\(com\.google\.security\.wycheproof\.[a-zA-z0-9]*\))")

# Golang specific regular expressions.
GOLANG_DIVISION_BY_ZERO_REGEX = re.compile(r"^panic: runtime error: integer divide by zero.*")
GOLANG_INDEX_OUT_OF_RANGE_REGEX = re.compile(r"^panic: runtime error: index out of range.*")
GOLANG_INVALID_MEMORY_ADDRESS_REGEX = re.compile(r"^panic: runtime error: invalid memory address.*")
GOLANG_MAKESLICE_LEN_OUT_OF_RANGE_REGEX = re.compile(r"^panic: runtime error: makeslice: len out of range.*")
GOLANG_SLICE_BOUNDS_OUT_OF_RANGE_REGEX = re.compile(r"^panic: runtime error: slice bounds out of range.*")
...
GOLANG_STACK_FRAME_FUNCTION_REGEX = re.compile(r"^([0-9a-zA-Z\.\-\_\\\/\(\)\*]+)\([x0-9a-f\s,\.{}]*\)$")
...
def format_address_to_dec(address, base=16):
    ...
    logs.warn("Error formatting address %s to decimal int64 in bases %s." % (str(address), str(try_bases)))
    return None
...
CHROME_STACK_FRAME_SPEC = StackFrameSpec(address=3, function_name=4)
...
CHROME_MAC_STACK_FRAME_SPEC = StackFrameSpec(address=5, function_name=6, function_offset=7, module_name=2, base=10)
...
WINDOWS_CDB_STACK_FRAME_SPEC = StackFrameSpec(address=1, function_name=4, function_offset=5, module_name=3)
...
LINUX_KERNEL_LIBRARY_ASSERT_REGEX = re.compile(r"([^:]+): lib/posix-host\.c:\d+: void panic\(void\): Assertion .*")
