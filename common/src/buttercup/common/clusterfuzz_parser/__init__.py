...
from buttercup.common.clusterfuzz_parser import crash_analyzer, utils
...
class StackParser:
    ...
    def __init__(
        ...
        self.stack_frame_ignore_regex = re.compile(
            r"(%s)" % "|".join(STACK_FRAME_IGNORE_REGEXES + custom_stack_frame_ignore_regexes)
        )
    ...
    def ignore_stack_frame(self, stack_frame):
        ...
        if self.symbolized and self.stack_frame_ignore_regex_if_symbolized.match(normalized_stack_frame):
            return True
    ...
    def add_frame_on_match(
        ...
        if demangle and environment.is_posix():
            pipe = subprocess.Popen(["c++filt", "-n", frame], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
        ...
        if frame_spec is not None:
            ...
            if frame_struct and frame_struct.module_offset and not frame_struct.function_name:
    ...
    def update_state_on_check_failure(self, state, line, regex, crash_type):
        ...
        check_match = self.update_state_on_match(regex, line, state, new_type=crash_type, reset=True, new_frame_count=1)
        if check_match:
    ...
    def match_assert(self, line, state, regex, group=1):
        """Match an assert."""
        assert_match = self.update_state_on_match(regex, line, state, new_type="ASSERT", new_frame_count=1)
    ...
    def filter_crash_parameters(self, state):
        ...
        if self.fuzz_target and state.crash_type in ["Out-of-memory", "Timeout", "Overwrites-const-input"]:
            state.crash_state = self.fuzz_target

        # Add a trailing \n if it does not exist in crash state.
        if state.crash_state and state.crash_state != "NULL" and state.crash_state[-1] != "\n":
            state.crash_state += "\n"
        ...
        if m:
            ...
            state.crash_type = state.crash_type[: len(m.group(1))] + str(num) + state.crash_type[m.end() :]
    ...
    def split_stacktrace(stacktrace: str):
        ...
        stacktrace = re.sub(CONCATENATED_SAN_DEADLYSIGNAL_REGEX, SPLIT_CONCATENATED_SAN_DEADLYSIGNAL_REGEX, stacktrace)
    ...
    def parse(self, stacktrace: str) -> CrashInfo:
        ...
        # line by line.
        if state.is_lkl:
            stacktrace = self.remove_lkl_kernel_times_and_set_params(state, stacktrace)
        ...
        for line in split_crash_stacktrace:
            ...
            # unknown address.
            self.update_state_on_match(ASSERT_NOT_REACHED_REGEX, line, state, new_type="ASSERT_NOT_REACHED", reset=True)

            # Platform specific: Linux gdb crash type format.
            self.update_state_on_match(
                LINUX_GDB_CRASH_TYPE_REGEX, line, state, type_from_group=1, type_filter=lambda s: s.upper()
            )

            # Platform specific: Linux gdb crash address format.
            self.update_state_on_match(LINUX_GDB_CRASH_ADDRESS_REGEX, line, state, address_from_group=1)

            # Platform specific: Linux gdb crash address format no registers
            self.update_state_on_match(LINUX_GDB_CRASH_ADDRESS_NO_REGISTERS_REGEX, line, state, address_from_group=1)

            # Platform specific: Mac gdb style crash address format.
            self.update_state_on_match(MAC_GDB_CRASH_ADDRESS_REGEX, line, state, address_from_group=1)
            ...
            if " suppressions" not in line and " warnings" not in line:
                self.update_state_on_match(
                    MSAN_TSAN_REGEX, line, state, reset=True, type_from_group=2, type_filter=lambda s: s.capitalize()
                )
            ...
            if not state.crash_type or state.crash_type == "Indirect-leak":
                self.update_state_on_match(LSAN_DIRECT_LEAK_REGEX, line, state, new_type="Direct-leak", reset=True)
            ...
            if not state.crash_type:
                self.update_state_on_match(LSAN_INDIRECT_LEAK_REGEX, line, state, new_type="Indirect-leak", reset=True)
            ...
            if not state.crash_type and self.include_ubsan:
                ubsan_vptr_match = self.update_state_on_match(
                    UBSAN_VPTR_REGEX, line, state, new_type="Bad-cast", new_frame_count=0, address_from_group=3
                )
                if ubsan_vptr_match:
                    state.crash_state = "Bad-cast to %s" % (ubsan_vptr_match.group(4)).strip("'")
            ...
            if state.crash_type == "Bad-cast" and not state.found_bad_cast_crash_end_marker:
                ...
                if UBSAN_VPTR_INVALID_VPTR_REGEX.match(line) or CFI_INVALID_VPTR_REGEX.match(line):
                ...
                if invalid_offset_match:
                    state.crash_state += " from base class subobject at offset %s" % invalid_offset_match.group(1)
                    state.found_bad_cast_crash_end_marker = True
            ...
            if not state.crash_type:
                ...
                if cfi_bad_cast_match:
                    state.crash_state = "Bad-cast to %s" % (cfi_bad_cast_match.group(2).strip("'"))
            ...
            if not state.crash_type:
                self.update_state_on_match(
                    CFI_NODEBUG_ERROR_MARKER_REGEX, line, state, new_type="Bad-cast", new_frame_count=0
                )
            ...
            if ubsan_runtime_match and not state.crash_type and self.include_ubsan:
                ...
                for ubsan_crash_regex, ubsan_crash_type in UBSAN_CRASH_TYPES_MAP:
                    if self.update_state_on_match(ubsan_crash_regex, reason, state, new_type=ubsan_crash_type):
                        break

                if state.crash_type == "UNKNOWN":
                    logs.error(f"Unknown UBSan crash type: {reason}")
            ...
            if state.is_golang:
                for golang_crash_regex, golang_crash_type in GOLANG_CRASH_TYPES_MAP:
                    if self.update_state_on_match(golang_crash_regex, line, state, new_type=golang_crash_type):
            ...
            if state.is_python:
                for python_crash_regex, python_crash_type in PYTHON_CRASH_TYPES_MAP:
                    if self.update_state_on_match(python_crash_regex, line, state, new_type=python_crash_type):
            ...
            if segv_match:
                ...
                if "ASSERT" in state.crash_type:
                    ...
                    int_crash_address = crash_analyzer.address_to_integer(temp_crash_address)
                    if crash_analyzer.is_assert_crash_address(int_crash_address) or SAN_SIGNAL_REGEX.match(stacktrace):
                        continue
            ...
            if self.update_state_on_match(
                ASAN_INVALID_FREE_REGEX, line, state, new_type="Invalid-free", reset=True, address_from_group=1
            ):
                continue

            # AddressSanitizer double free crash.
            if self.update_state_on_match(
                ASAN_DOUBLE_FREE_REGEX, line, state, new_type="Heap-double-free", reset=True, address_from_group=3
            ):
                continue

            # Sanitizer floating point exception.
            if self.update_state_on_match(SAN_FPE_REGEX, line, state, new_type="Floating-point-exception", reset=True):
                continue
            ...
            if (
                not state.crash_type.startswith("Kernel failure")
                and not state.found_golang_crash
                and not state.found_python_crash
            ):
                self.update_state_on_match(
                    SAN_ADDR_REGEX,
                    line,
            ...
            self.update_state_on_match(WINDOWS_SAN_ILL_REGEX, line, state, new_type="Ill", reset=False)
            ...
            state_needs_change = not state.crash_type.startswith("UNKNOWN") or "Fatal signal" not in line
            if state_needs_change:
                ...
                if android_segv_match:
                    ...
                    process_name_match = ANDROID_PROCESS_NAME_REGEX.match(android_segv_match.group(2))
            ...
            # KASan GPFs.
            self.update_state_on_match(
                KASAN_GPF_REGEX, line, state, new_type="Kernel failure\nGeneral-protection-fault"
            )

            # GPU Failure.
            self.update_state_on_match(
                GPU_PROCESS_FAILURE, line, state, new_type="GPU failure", new_state="", reset=True
            )

            # Command injection bugs detected by extra sanitizers.
            self.update_state_on_match(
                EXTRA_SANITIZERS_COMMAND_INJECTION_REGEX, line, state, new_type="Command injection"
            )

            # Arbitrary file open detected by extra sanitizers.
            self.update_state_on_match(
                EXTRA_SANITIZERS_ARBITRARY_FILE_OPEN_REGEX, line, state, new_type="Arbitrary file open"
            )

            # Arbitrary DNS resolution detected by extra sanitizers.
            self.update_state_on_match(EXTRA_SANITIZERS_ARBITRARY_DNS, line, state, new_type="Arbitrary DNS resolution")

            # Issue detected by PySecSan
            self.update_state_on_match(EXTRA_SANITIZERS_PYSECSAN, line, state, new_type="PySecSan")
            ...
            self.update_state_on_check_failure(state, line, SECURITY_CHECK_FAILURE_REGEX, "Security CHECK failure")
            self.update_state_on_check_failure(state, line, SECURITY_DCHECK_FAILURE_REGEX, "Security DCHECK failure")
            ...
            if self.detect_ooms_and_hangs:
                for timeout_regex in [LIBFUZZER_TIMEOUT_REGEX, CENTIPEDE_TIMEOUT_REGEX]:
                    self.update_state_on_match(timeout_regex, line, state, new_type="Timeout", reset=True)
                self.update_state_on_match(OUT_OF_MEMORY_REGEX, line, state, new_type="Out-of-memory", reset=True)

            # The following parsing signatures don't lead to crash state overwrites.
            if not state.crash_type:
                # Windows cdb stack overflow.
                self.update_state_on_match(WINDOWS_CDB_STACK_OVERFLOW_REGEX, line, state, new_type="Stack-overflow")

                # Windows cdb generic type regex.
                self.update_state_on_match(
                    WINDOWS_CDB_CRASH_TYPE_REGEX, line, state, type_from_group=1, type_filter=fix_win_cdb_crash_type
                )

                # Generic ASan regex.
                self.update_state_on_match(
                    ASAN_REGEX, line, state, reset=True, type_from_group=2, type_filter=fix_sanitizer_crash_type
                )

                # HWASan object address for allocation tail overwritten is on same line
                # as crash type, so add it here.
                self.update_state_on_match(
                    HWASAN_ALLOCATION_TAIL_OVERWRITTEN_ADDRESS_REGEX, line, state, address_from_group=1
                )

                if self.update_state_on_match(
                    JAZZER_JAVA_SECURITY_EXCEPTION_REGEX, line, state, new_type="Security exception"
                ) or self.update_state_on_match(
                    JAZZER_JAVA_EXCEPTION_REGEX, line, state, new_type="Uncaught exception"
                ):
                    state.found_java_exception = True

                if self.update_state_on_match(
                    WYCHEPROOF_JAVA_EXCEPTION, line, state, new_type="Wycheproof error", state_from_group=1
                ):
                    state.found_java_exception = True

                # Android fatal exceptions.
                if self.update_state_on_match(
                    ANDROID_FATAL_EXCEPTION_REGEX, line, state, new_type="Fatal Exception", reset=True
                ):
                    state.found_java_exception = True

                # Check failures.
                self.update_state_on_check_failure(state, line, GOOGLE_LOG_FATAL_REGEX, "Fatal error")
                self.update_state_on_check_failure(state, line, CHROME_CHECK_FAILURE_REGEX, "CHECK failure")
                self.update_state_on_check_failure(state, line, GOOGLE_CHECK_FAILURE_REGEX, "CHECK failure")
                ...
                if state.is_golang:
                    golang_fatal_error_match = self.update_state_on_match(
                        GOLANG_FATAL_ERROR_REGEX, line, state, new_type="Fatal error", reset=True
                    )
                ...
                if self.detect_v8_runtime_errors:
                    runtime_error_match = self.update_state_on_match(
                        RUNTIME_ERROR_REGEX, line, state, new_type="RUNTIME_ASSERT", reset=True
                    )
                    if runtime_error_match:
                        state.crash_state = _filter_stack_frame(runtime_error_match.group(1))
                        state.fatal_error_occurred = True
                ...
                self.update_state_on_match(
                    V8_CORRECTNESS_FAILURE_REGEX, line, state, new_type="V8 correctness failure", reset=True
                )
                ...
                self.update_state_on_match(
                    LIBFUZZER_DEADLY_SIGNAL_REGEX, line, state, new_type="Fatal-signal", reset=True
                )

                # Libfuzzer fuzz target exited errors.
                self.update_state_on_match(
                    LIBFUZZER_FUZZ_TARGET_EXITED_REGEX, line, state, new_type="Unexpected-exit", reset=True
                )

                # Libfuzzer fuzz target overwrites const input errors.
                self.update_state_on_match(
                    LIBFUZZER_OVERWRITES_CONST_INPUT_REGEX, line, state, new_type="Overwrites-const-input", reset=True
                )
            ...
            if state.fatal_error_occurred:
                ...
                if error_line_match:
                    state.check_failure_source_file = fix_filename_string(error_line_match.group(1))
                ...
                check_failure_match = self.update_state_on_match(
                    FATAL_ERROR_DCHECK_FAILURE, line, state, new_type="DCHECK failure", reset=True
                )

                if not check_failure_match:
                    check_failure_match = self.update_state_on_match(
                        FATAL_ERROR_CHECK_FAILURE, line, state, new_type="CHECK failure", reset=True
                    )
                ...
                if check_failure_match and check_failure_match.group(2).strip():
                    failure_string = fix_check_failure_string(check_failure_match.group(2))
                    if state.check_failure_source_file:
                        state.crash_state = "%s in %s\n" % (failure_string, state.check_failure_source_file)
                    else:
                        state.crash_state = "%s\n" % failure_string
                    state.frame_count = 1
                ...
                if state.check_failure_source_file:
                    new_state = "%s\n" % state.check_failure_source_file
                self.update_state_on_match(
                    FATAL_ERROR_UNREACHABLE, line, state, new_state=new_state, new_type="Unreachable code", reset=True
                )

            # Check cases with unusual stack start markers.
            self.update_state_on_match(WINDOWS_CDB_STACK_START_REGEX, line, state, new_state="", new_frame_count=0)
            ...
            if self.add_frame_on_match(
                WINDOWS_CDB_STACK_FRAME_REGEX, line, state, group=4, frame_spec=WINDOWS_CDB_STACK_FRAME_SPEC
            ):
                continue
            ...
            if self.add_frame_on_match(
                CHROME_STACK_FRAME_REGEX, line, state, group=4, frame_spec=CHROME_STACK_FRAME_SPEC
            ):
                continue
            ...
            if self.add_frame_on_match(
                CHROME_WIN_STACK_FRAME_REGEX, line, state, group=1, frame_spec=CHROME_WIN_STACK_FRAME_SPEC
            ):
                continue
            ...
            if state.found_java_exception:
                if state.crash_type in [
                    ...
                    self.add_frame_on_match(JAVA_EXCEPTION_CRASH_STATE_REGEX, line, state, group=1)
                    continue

            # Android kernel stack frame.
            android_kernel_match = self.add_frame_on_match(ANDROID_KERNEL_STACK_FRAME_REGEX, line, state, group=3)
            ...
            if self.add_frame_on_match(ANDROID_KERNEL_STACK_FRAME_NO_ADDRESS_REGEX, line, state, group=2):
                state.found_android_kernel_crash = True
                continue

            # V8 correctness fuzzer metadata.
            if self.add_frame_on_match(V8_CORRECTNESS_METADATA_REGEX, line, state, group=1, frame_filter=lambda s: s):
                continue

            # Golang stack frames.
            if state.is_golang and self.add_frame_on_match(
                GOLANG_STACK_FRAME_FUNCTION_REGEX, line, state, group=1, frame_filter=lambda s: s.split("/")[-1]
            ):
                continue

            # Python stack frames.
            if state.is_python and self.add_frame_on_match(PYTHON_STACK_FRAME_FUNCTION_REGEX, line, state, group=3):
                continue
    ...
    for frame_index in range(num_frames):
        for cycle_length in range(1, MAX_CYCLE_LENGTH + 1):
            ...
            potential_cycles_are_equal = all(
                frame_potential_cycle == frame_potential_cycles[0] for frame_potential_cycle in frame_potential_cycles
            )
...
def llvm_test_one_input_override(frame, frame_struct):
    ...
    if frame_struct and frame_struct.filename:
        ...
        return frame.replace("LLVMFuzzerTestOneInput", os.path.basename(frame_struct.filename.replace("\\", "/")))
