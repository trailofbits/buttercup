...
from buttercup.seed_gen.seed_explore import SeedExploreState, SeedExploreTask
from buttercup.seed_gen.task import CodeSnippet
...
class TestSeedExploreTask:
    ...
    def test_init(
        self, seed_explore_task, mock_challenge_task, mock_codequery, mock_project_yaml, mock_redis
    ):
    ...
    def test_get_context_generates_proper_command(
        self, seed_explore_task, mock_harness_info, mock_code_snippet, temp_output_dir
    ):
    ...
    def test_generate_seeds_creates_proper_command(
        ...
        with patch.object(
            seed_explore_task, "_generate_python_funcs_base", return_value=mock_generated_functions
        ):
    ...
    def test_generate_seeds_workflow_execution(
        self, seed_explore_task, mock_harness_info, mock_code_snippet, temp_output_dir
    ):
        ...
        with patch.object(seed_explore_task, "_build_workflow", return_value=mock_workflow):
            with patch("buttercup.seed_gen.seed_explore.get_langfuse_callbacks", return_value=[]):
                with patch("buttercup.seed_gen.seed_explore.trace") as mock_trace:
                    ...
                    seed_explore_task.generate_seeds(
                        mock_harness_info, mock_code_snippet, temp_output_dir
                    )
    ...
    def test_get_function_def_success(self, seed_explore_task, mock_target_function):
        ...
        with patch.object(
            seed_explore_task, "_do_get_function_def", return_value=mock_target_function
        ):
    ...
    def test_get_function_def_failure(self, seed_explore_task):
        ...
        with patch.object(seed_explore_task, "_do_get_function_def", return_value=None):
            result = seed_explore_task.get_function_def(
                "nonexistent_function", [Path("/test/target.c")]
            )
            assert result is None

    def test_do_task_success(
        self, seed_explore_task, mock_harness_info, mock_target_function, temp_output_dir
    ):
        ...
        with patch(
            ...
            with patch.object(
                seed_explore_task, "get_function_def", return_value=mock_target_function
            ):
                with patch.object(seed_explore_task, "generate_seeds") as mock_generate_seeds:
                    seed_explore_task.do_task(
                        "target_function", [Path("/test/target.c")], temp_output_dir
                    )
            ...
            with patch.object(seed_explore_task, "get_function_def", return_value=None):
                with patch.object(seed_explore_task, "generate_seeds") as mock_generate_seeds:
                    with patch("buttercup.seed_gen.seed_explore.logger") as mock_logger:
                        seed_explore_task.do_task(
                            "nonexistent_function", [Path("/test/target.c")], temp_output_dir
                        )
    ...
    def test_do_task_exception_handling(
        ...
        with patch(
            ...
            with patch.object(
                ...
                with patch.object(
                    seed_explore_task, "generate_seeds", side_effect=Exception("Test error")
                ):
                    with patch("buttercup.seed_gen.seed_explore.logger") as mock_logger:
                        seed_explore_task.do_task(
                            "target_function", [Path("/test/target.c")], temp_output_dir
    ...
    def test_seed_explore_state_creation(
        self, mock_harness_info, mock_code_snippet, seed_explore_task, temp_output_dir
    ):
    ...
    def test_continue_context_retrieval(
        self, seed_explore_task, mock_harness_info, mock_code_snippet, temp_output_dir
    ):
