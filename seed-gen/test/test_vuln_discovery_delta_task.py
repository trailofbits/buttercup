...
from buttercup.seed_gen.vuln_discovery_delta import VulnDiscoveryDeltaState, VulnDiscoveryDeltaTask
...
class TestVulnDiscoveryDeltaTask:
    ...
    def test_analyze_bug_creates_proper_command(
        ...
        mock_analysis = (
            "This diff introduces a buffer overflow vulnerability in the target function."
        )
    ...
    def test_init_state_success(
        self, vuln_discovery_delta_task, mock_harness_info, temp_output_dir, temp_current_dir
    ):
        """Test _init_state success case."""
        with patch.object(
            vuln_discovery_delta_task, "get_harness_source", return_value=mock_harness_info
        ):
            with patch.object(vuln_discovery_delta_task, "sample_sarifs", return_value=[]):
                with patch(
                    "buttercup.seed_gen.vuln_discovery_delta.get_diff_content",
                    return_value="test diff content",
                ):
    ...
    def test_init_state_no_harness(
        self, vuln_discovery_delta_task, temp_output_dir, temp_current_dir
    ):
    ...
    def test_init_state_no_diff(
        ...
        with patch.object(
            ...
            with patch(
                "buttercup.seed_gen.vuln_discovery_delta.get_diff_content", return_value=None
            ):
                with pytest.raises(ValueError, match="No diff found"):
                    vuln_discovery_delta_task._init_state(temp_output_dir, temp_current_dir)

    def test_delta_state_diff_content_field(
        self, mock_harness_info, temp_output_dir, temp_current_dir
    ):
    ...
    def test_get_diff_content_usage(
        self, vuln_discovery_delta_task, mock_harness_info, temp_output_dir, temp_current_dir
    ):
    ...
    def test_submit_valid_pov_delta_mode(
        self, vuln_discovery_delta_task, mock_build_output, mock_reproduce_result, temp_output_dir
    ):
        ...
        with patch(
            ...
            vuln_discovery_delta_task.submit_valid_pov(
    ...
    def test_test_povs_with_delta_context(
        self,
        vuln_discovery_delta_task,
        mock_build_output,
    ...
    def test_prompt_variables_include_diff(
        self,
        vuln_discovery_delta_task,
        mock_harness_info,
        mock_sarifs,
    ...
    def test_do_task_success(
        ...
        with patch(
            ...
            with patch.object(vuln_discovery_delta_task, "_init_state") as mock_init_state:
                with patch.object(
                    vuln_discovery_delta_task, "_build_workflow"
                    ...
                    with patch(
                        "buttercup.seed_gen.vuln_discovery_delta.get_langfuse_callbacks",
                        return_value=[],
                    ):
    ...
    def test_do_task_exception_handling(
        ...
        with patch.object(
            vuln_discovery_delta_task, "_init_state", side_effect=Exception("Test error")
        ):
