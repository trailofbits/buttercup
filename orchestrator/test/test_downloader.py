...
@responses.activate
def test_download_source_success(downloader, sample_task):
    ...
    # Test the download
    result = downloader.download_source(task.task_id, source)
...
@responses.activate
def test_download_source_wrong_hash(downloader, sample_task):
    task, content = sample_task
    source = task.sources[0]
...
def test_download_and_extract_tar(downloader, sample_tar_task):
    ...
    extracted_file = downloader.get_source_type_dir(task.task_id, source.source_type) / "test.txt"
    assert extracted_file.exists()
    assert extracted_file.read_bytes() == b"test file content"

    # Verify the tar file was removed after extraction
    tar_file = downloader.get_source_type_dir(task.task_id, source.source_type) / "test.tar.gz"
    assert not tar_file.exists()


def test_get_source_type_dir(downloader):
    task_id = "test_task"
    source_type = SourceDetail.SourceType.SOURCE_TYPE_REPO

    dir_path = downloader.get_source_type_dir(task_id, source_type)

    assert dir_path.exists()
    assert dir_path.is_dir()
    assert str(dir_path).endswith(f"{task_id}/repo")
...
def test_process_task_with_multiple_sources(downloader):
    ...
    for i in range(2):
        file_path = (
            downloader.get_source_type_dir(task.task_id, SourceDetail.SourceType.SOURCE_TYPE_REPO) / f"file{i}.txt"
        )
