...
from gremlin_python.process.graph_traversal import Transaction
import sys
import argparse
import base64
...
class KytheURI:
    ...
    def from_vname(vn: VName):
        return KytheURI(signature=vn.signature, corpus=vn.corpus, root=vn.root, path=vn.path, language=vn.language)
    ...
    def process_stream(self, _project_name: str, fl: BytesIO):
        ...
        for ent in iterate_over_entries(fl):
            ...
            if is_edge(ent):
                ...
                trv.add_e().from_(n1).to(n2).property("edge_kind", ent.edge_kind).iterate()
            else:
                n1 = convert_node(trv, ent.source)
                trv.V().has_id(n1.id).property(ent.fact_name, ent.fact_value.decode()).iterate()
        tx.commit()
