...
def _find_source_files(
    task_dir: Path, file_patterns: list[str], grep_pattern: str
) -> list[Path]:
...
def find_libfuzzer_harnesses(task_dir: Path) -> list[Path]:
...
def find_jazzer_harnesses(task_dir: Path) -> list[Path]:
...
def _rebase_path(task_dir: Path, path: Path) -> Path:
    ...
    try:
        ...
        return Path("/src", *res.parts)
    except ValueError:
        return path.absolute()


def get_harness_source_candidates(
    challenge_task: ChallengeTask, harness_name: str
) -> list[Path]:
...
def get_harness_source(
    ...
    if len(harnesses) == 1:
        ...
        harness_info = HarnessInfo(
            file_path=_rebase_path(challenge_task.task_dir, harnesses[0]),
    ...
    if redis is None:
        ...
        coverage_map = CoverageMap(
            redis, harness_name, challenge_task.project_name, task_id
        )
    ...
    for function_coverage in function_coverages:
        for harness_path in harnesses:
            rebased_harness_path = _rebase_path(challenge_task.task_dir, harness_path)
            if any(
    ...
    rebased_harness_path = _rebase_path(challenge_task.task_dir, harnesses[0])
    harness_info = HarnessInfo(
        file_path=rebased_harness_path,
        code=harnesses[0].read_text(),
        harness_name=harness_name,
