...
def chunk_data(generator, chunk_size):
...
class GraphWriter:
    ...
    def write_entry(self, entries: list[bytes]):
        node_props = list()
        edge_props = list()
        edges = list()
        nodes = list()
...
class GraphStorage:
    ...
    def process_stream(self, fl: BytesIO, outfile):
    ...
    def to_graphml(self, outfile: TextIO, nodes: list[Node], edges: list[Edge]):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--graphml", type=str, required=True)
    parser.add_argument("--task_id", type=str, required=True)
    parser.add_argument("--bin", type=str, required=True)
    args = parser.parse_args()

    gs = GraphStorage(args.task_id)
    with open(args.graphml, "w") as outfile:
        with open(args.bin, "rb") as f:
            gs.process_stream(f, outfile)
