...
@pytest.mark.parametrize(
    "function_name,file_path,line_number,fuzzy,expected_callees",
    [
        (
            "jpeg_skip_scanlines",
            "src/libjpeg-turbo/jdapistd.c",
...
@pytest.mark.integration
# NOTE(boyan): this is similar to the test_libjpeg_get_callers
...
def test_libjpeg_get_callees(
    ...
    # NOTE(boyan): disabled for now because it fails. The problem
    # in this example is that there are 3 start_iMCU_row functions
    ...
    # assert len(expected_callees) == len(callees)
