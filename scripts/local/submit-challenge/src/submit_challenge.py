...
def prepare_challenge(
    ...
    # Create fuzz-tooling tarball (just the projects directory)
    if (source_dir / "projects").exists():
        fuzz_tarball = work_dir / "fuzz-tooling.tar.gz"
        sha256 = create_tarball(source_dir / "projects", fuzz_tarball, base_name="projects")
...
def submit_task(
    ...
    task_detail = TaskDetail(
        ...
        harnesses_included=False,  # Let the system build harnesses
        metadata={
    ...
    with tempfile.TemporaryDirectory() as work_dir:
        ...
        try:
            ...
            try:
                ...
                # Keep server running for a bit to allow downloads
                logger.info("Keeping file server running for 60 seconds to allow downloads...")
                time.sleep(60)
