...
Run the automated triage snapshot from the repo root:
...
```bash
bash deployment/diagnose.sh --full
```

Override the namespace with `BUTTERCUP_NAMESPACE` if the deployment is not in `crs`.
