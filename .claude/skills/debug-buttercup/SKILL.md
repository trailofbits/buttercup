...
description: >
  ...
  task-server, program-model, litellm, dind-daemon, tracer-bot, merger-bot.
---
...
| Layer | Services |
|-------|----------|
| Infra | redis, dind-daemon, litellm |
...
- **DinD**: `kubectl logs -n crs -l app=dind-daemon --tail=100` — look for docker daemon crashes, storage driver errors
...
Run `scripts/diagnose.sh` for an automated triage snapshot:

```bash
bash .claude/skills/debug-buttercup/scripts/diagnose.sh
```
