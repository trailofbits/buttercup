...
class CoverageRunner:
    ...
    def run(self,  harness_name: str, corpus_dir: str, package_name: str) -> dict| None:
        ...
        if not profdata_path.exists():
            logger.error(f"Failed to find profdata for {harness_name} | {corpus_dir} | {package_name} | in {profdata_path}")
            return False
        ...
        if ret.returncode != 0:
            logger.error(f"Failed to convert profdata to json for {harness_name} | {corpus_dir} | {package_name} | in {coverage_file}")
            return False
